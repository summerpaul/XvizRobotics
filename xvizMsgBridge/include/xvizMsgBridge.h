 ...
 * @Last Modified time: 2024-01-29 20:20:24
 */
#include <stdint.h>
...
public:
  ...
  /// @param sub_connect 接受数据需要绑定的zmq Ip与端口
  /// （目前实现接受起点与终点位姿）
  /// @return
  ...
  void PolygonArrayPub(const std::string &topic,
                       const Polygon2fArray &polygonArray);
   ...
   * @param file_prefix
   */
  void OpenCsvFile(const std::string &file_prefix);
...
private:
  ...
  void PubProto(const PROTO_MSG &proto_msg, const std::string &topic,
